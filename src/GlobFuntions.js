...
export const formattingPhone = function (val, fn) {
    if (!val) return ''
...
export function numberNoSpace(x) { //убрать проблеы среди числе
    if (!x) return 0
...
export function checkEmptyFields(formEl) {
    return formEl && formEl.validate((valid, a) => {
...
export function checkVIN(rule, value, callback) {
...
export function rotatePhoto(base64Data, direction, height, width) {
...
export function checkMili(rule, value, callback) {
